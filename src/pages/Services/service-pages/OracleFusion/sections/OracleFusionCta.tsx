import type { OracleFusionCtaData } from "../oracle-fusion.data";

export default function OracleFusionCta({ data }: { data: OracleFusionCtaData }) {
  return (
    <section className="w-full bg-bg py-20">
      <div className="mx-auto w-full max-w-5xl px-6 lg:px-10">
        <div className="relative overflow-hidden rounded-3xl border border-border bg-gradient-to-br from-[rgb(var(--color-accent-purple))] via-[rgb(var(--color-accent-purple))] to-[rgb(var(--color-accent-cyan))] p-10 text-center shadow-card sm:p-14">
          <div aria-hidden className="pointer-events-none absolute inset-0">
            <div className="absolute -left-24 -top-24 h-72 w-72 rounded-full bg-white/15 blur-3xl" />
            <div className="absolute -right-24 -bottom-24 h-72 w-72 rounded-full bg-[rgb(var(--color-accent-magenta))]/30 blur-3xl" />
          </div>
